async function loadStats() {
      ...
      function generateChartLegendHTML() {
        return '<div class="chart-legend" style="margin-top: 10px; font-size: 12px;">' +
      ...
      function generateGlobalChartHTML(stats) {
        ...
        let html = '<div style="width: 100%; height: 30px; position: relative; border: 1px solid var(--color-border); border-radius: 3px; overflow: hidden; margin-bottom: 5px;">';
      ...
      async function loadTimeline() {
        try {
          ...
          if (timelineData.length === 0) {
            document.getElementById("timeline-chart").innerHTML = "<p>Aucune donnée disponible pour le timeline.</p>";
            return;
          }
          ...
          // Group data by time unit - simplified logic
          const groupedData = {};
          timelineData.forEach(entry => {
            ...
            if (!groupedData[groupKey]) {
              ...
              const displayDate = useWeekly ? new Date(groupKey) : new Date(entry.date);
          ...
          let html = '<div style="display: flex; overflow-x: auto; gap: 5px; margin-bottom: 10px;">';
          ...
          groupedArray.forEach(period => {
            ...
            html += '<div style="width: 30px; height: 100px; position: relative; border: 1px solid var(--color-border); border-radius: 3px; overflow: hidden; margin: 0 auto;">';
            ...
            if (period.delay_45min > 0) {
              ...
              const bottom = (period.on_time + period.delay_5min + period.delay_15min) * segmentHeight;
            ...
            // Over 45 min (top segment - reversed)
            if (period.delay_over_45min > 0) {
              const height = period.delay_over_45min * segmentHeight;
              const bottom = (period.on_time + period.delay_5min + period.delay_15min + period.delay_45min) * segmentHeight;
          ...
          document.getElementById("timeline-chart").innerHTML = "<p>Erreur lors du chargement du timeline.</p>";
        }
      }
